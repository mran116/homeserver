...
#   - Sonarr parser-mismatch sweep (sonarr-bulk-import.py --apply) every 30 min —
#     force-imports queue items Sonarr blocked because the filename re-parses to
#     a different TVDB series than the grab history (unified anthology entries
#     like Power Rangers). No-op when the queue is clean.
#
...
sf_marker="# homestack-sonarr-bulk-import"
sf_line="*/30 * * * * cd $REPO_DIR && ./scripts/sonarr-bulk-import.py --apply >> $REPO_DIR/sonarr-bulk-import.log 2>&1 $sf_marker"
...
plan_line "$sf_marker" "$sf_line" "Sonarr parser-mismatch sweep cron (every 30 min) → sonarr-bulk-import.log"

show_plan || exit 0
gate || exit 0

new_cron="$( { printf '%s\n' "$cron_now" | grep -vF "$ks_marker" | grep -vF "$pr_marker" | grep -vF "$sw_marker" | grep -vF "$sf_marker"; echo "$ks_line"; echo "$pr_line"; echo "$sw_line"; echo "$sf_line"; } )"
