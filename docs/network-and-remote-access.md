...
> Concrete hostnames below use **`mranlab.com`** as the example domain. The real
...
- **HA behind the reverse proxy** at `home.mranlab.com` (see TLS below). Set in
  HA's `configuration.yaml`:
  ```yaml
  homeassistant:
    external_url: "https://home.mranlab.com"
    internal_url: "https://home.mranlab.com"
  http:
...
- **Service integrations use hostnames, not `IP:port`.** Point HACS
  ...
  Uptime Kuma) at `*.mranlab.com` names so they survive IP changes and get valid
...
1. **Tailscale (private, everything)** — the **Flint 3 acts as a subnet router**
   ...
   **split-DNS** pointing `mranlab.com` at AdGuard so the same names resolve over
...
|---|---|---|---|---|
| Jellyfin | yes | Direct (DNS-only A + 443) **+** Tailscale | `jellyfin.mranlab.com` | `http://jellyfin:8096` |
| Seerr | yes | Cloudflare Tunnel | `requests.mranlab.com` | `http://seerr:5055` |
| Immich | yes | Cloudflare Tunnel | `photos.mranlab.com` | `http://immich-server:3001` |
| Audiobookshelf | yes | Cloudflare Tunnel | `audiobooks.mranlab.com` | `http://audiobookshelf:80` |
| Navidrome | yes | Cloudflare Tunnel | `music.mranlab.com` | `http://navidrome:4533` |
...
**Phase 1 — when `mranlab.com` is live (tomorrow)**
- [ ] Add `mranlab.com` to Cloudflare; create a scoped API token (Zone:DNS:Edit)
...
- [ ] Cloudflare Access: policy on `vault.mranlab.com` path `/admin`
...
- [ ] Add `dash.mranlab.com` (and any new proxied host) to
